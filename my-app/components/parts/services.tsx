...
const SERVICES = [
  {
    ...
    description: 'Experience the premium Zena Facial, a luxurious 60-minute treatment that combines advanced techniques with premium skincare products. This rejuvenating facial deeply cleanses, exfoliates, and hydrates your skin, leaving you with a radiant, youthful glow.',
    duration: '60 mins',
...
export default function ParallaxServices() {
  ...
  return (
    <section ref={containerRef} id="services" className="relative bg-[#F5F0E9] py-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-32 space-y-6">
          ...
          >
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#3C507D]">Curation</span>
          </motion.div>
          <motion.h2 
          ...
          >
            <Link href="#discovery" className="inline-flex items-center gap-2 bg-[#112250]/5 hover:bg-[#112250]/10 px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest text-[#3C507D] transition-colors border border-[#E0C58F]/10">
...
function ServiceCard({ service, index }: { service: any, index: number }) {
  ...
  return (
    <div ref={cardRef} className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center gap-20`}>
      ...
      <div className="w-full lg:w-3/5 relative">
        ...
        <motion.div 
          ...
          className={`absolute ${isEven ? '-right-10' : '-left-10'} top-1/2 -translate-y-1/2 bg-white/60 backdrop-blur-2xl p-8 rounded-3xl shadow-2xl border border-white/50 hidden lg:block`}
        >
          <p className="text-[#E0C58F] font-black text-xs uppercase tracking-[0.2em] mb-2">Duration</p>
          <p className="text-[#112250] text-3xl font-black">{service.duration}</p>
        </motion.div>
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-[#E0C58F] font-black text-sm uppercase tracking-[0.3em]">0{index + 1}.</p>
          <h3 className="text-5xl md:text-6xl font-black text-[#112250] leading-none" style={{ fontFamily: 'Playfair Display, serif' }}>
            {service.title}
          </h3>
        </div>
        
        <p className="text-xl text-[#3C507D] font-medium leading-relaxed italic" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
          "{service.description}"
        </p>
        ...
        <Link href={`/services/${service.slug}`}>
          <button className="group relative px-10 py-4 rounded-full font-black text-[#112250] uppercase tracking-[0.2em] text-[10px] overflow-hidden border-2 border-[#112250]/20 hover:border-[#112250] transition-colors duration-500">
            <span className="relative z-10 flex items-center gap-3">
              Explore Detail <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </span>
          </button>
        </Link>
      </motion.div>
